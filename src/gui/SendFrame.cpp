...
namespace WalletGui {
	...
	void SendFrame::parsePaymentRequest(QString _request) {
		if (_request.startsWith("hospitalcoin://", Qt::CaseInsensitive))
		{
			_request.replace(0, 13, "hospitalcoin:");
		}
		if (!_request.startsWith("hospitalcoin:", Qt::CaseInsensitive)) {
			QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with hospitalcoin:"), QtCriticalMsg));
			return;
		}
	...
	void SendFrame::sendClicked() {
		...
		if (dlg.exec() == QDialog::Accepted) {
			...
			if (m_ui->donateCheckBox->isChecked()) {
				...
				CryptoNote::WalletLegacyTransfer walletTransfer;
				walletTransfer.address = "hozYenRVzuxGxSfJWrJgLqTwtmjSvE4NDQFiPK25zPb4DJP5ZtFYDyDXJV9MAZ6PtqPBQdaeXHNEAVf6qsmpnCsR1ytmJnkY1d";
