...
// Copyright (c) 2016-2017 The HospitalCoin developers
// Copyright (c) 2018 The HospitalCoin developers
...
int main(int argc, char* argv[]) {
	...
	QSettings hospitalcoinKey("HKEY_CLASSES_ROOT\\Hospitalcoin", QSettings::NativeFormat);
	hospitalcoinKey.setValue(".", "Hospitalcoin Wallet");
	hospitalcoinKey.setValue("URL Protocol", "");
	QSettings hospitalcoinOpenKey("HKEY_CLASSES_ROOT\\hospitalcoin\\shell\\open\\command", QSettings::NativeFormat);
	hospitalcoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
	...
	//as root
	args << "-c" << "printf '[Desktop Entry]\\nName = HOSP URL Handler\\nGenericName = hospitalcoin\\nComment = Handle URL Sheme hospitalcoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/hospitalcoin;\\nIcon = Hospitalcoin-Wallet' | tee /usr/share/applications/Hospitalcoin-handler.desktop";
