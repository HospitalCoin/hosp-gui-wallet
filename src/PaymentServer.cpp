...
// Copyright (c) 2016 The HospitalCoin developers
...
const QString BITCOIN_IPC_PREFIX("HospitalCoin:");

static QString ipcServerName()
{
    QString name("HospitalCoin");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start HospitalCoin: click-to-pay handler");
    else
...
{
    // clicking on HospitalCoin: URLs creates FileOpen events on the Mac:
